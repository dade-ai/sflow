...
def patch_to_space(x, imgsz, crops=None):
    ...
    data = x.reshape(batch, blocksz[0], blocksz[1], patchsz[0], patchsz[1], depth)
    ...
    data = data.reshape(batch, imgsz[0], imgsz[1], depth)
...
def space_to_patch(x, patchsz, paddings=None):
    ...
    data = x.reshape(batch, blocksz[0], patchsz[0], blocksz[1], patchsz[1], depth)
    # dim to [batch, block0, block1, patch0, patch1, d]
    data = data.transpose(0, 1, 3, 2, 4, 5)
    # new batch size
    batch *= (blocksz[0] * blocksz[1])
    data = data.reshape(batch, patchsz[0], patchsz[1], depth)
